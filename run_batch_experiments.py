#!/usr/bin/env python
"""
배치 사이즈(64, 256, 1024)에 따른 실험을 차례대로 실행하는 스크립트
"""
import os
import subprocess
import argparse
...
def main():
    ...
    parser.add_argument("--batch-sizes", type=str, default="1024,256,64",
                        help="실행할 배치 사이즈 목록 (쉼표로 구분, 기본값: 1024,256,64)")
    ...
    # 배치 사이즈를 역순으로 정렬 (큰 것부터 작은 것 순으로)
    batch_sizes.sort(reverse=True)
    
    print(f"배치 사이즈 실험을 시작합니다 (큰 배치부터): {batch_sizes}")
